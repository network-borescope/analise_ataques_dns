...
import datetime
from ip_to_nome_lat_lon import load_ips,site_from_ip
...
def is_client(ip):
    ...
    return result[0] # return client_name
...
# {
# ip_dst ou client_name: {
#   ip_src ou client_name: {
#     "req_count": 0, 
#     "req_sem_resp_count": 0,
#     "req_eof_count": 0,
#     "pairs_count": 0,
            ...
            if c >= '0' and c <= '9':
                ...
                if (data[D_PROTO] + ":" + data[D_SPORT] + ":" + port_dst) == "17:53:53": # conversa entre servidores DNS
                    ...
                    if items[6][0] >= '0' and items[6][0] <= '9':
                        ...
                        if data[D_DIP] in open_dns: target = open_dns[data[D_DIP]]
                        else: target = is_client(data[D_DIP]) # destino da req

                        if target not in dns_match:
                            dns_match[target] = {}

                        source = is_client(data[D_SIP]) # origem da req
                        if source not in dns_match[target]:
                            dns_match[target][source] = {}

                            dns_match[target][source][key] = [1, None, hour_to_timedelta(data[D_HORA])]

                            dns_match[target][source]["req_count"] = 1
                            dns_match[target][source]["req_sem_resp_count"] = 0
                            dns_match[target][source]["req_eof_count"] = 0
                            dns_match[target][source]["pairs_count"] = 0
                        
                        elif key not in dns_match[target][source]:
                            dns_match[target][source][key] = [1, None, hour_to_timedelta(data[D_HORA])]
                            dns_match[target][source]["req_count"] += 1
                    ...
                    if items[6][0] >= '0' and items[6][0] <= '9':
                        ...
                        if data[D_SIP] in open_dns: target = open_dns[data[D_SIP]]
                        else: target = is_client(data[D_SIP]) # destino da req/origem da resposta

                        source = is_client(data[D_DIP]) # origem da req/destino da resposta

                        if target in dns_match:
                            if source in dns_match[target]:
                                if key in dns_match[target][source] and dns_match[target][source][key][RESPONSE] is None:
                                    dns_match[target][source][key][RESPONSE] = 1
                                    dns_match[target][source]["pairs_count"] += 1
...
with open(f"{VERSION}_{filename}_estatisca_dns_por_ip.txt", "w") as fout:
    for target in dns_match:
        for source in dns_match[target]:
            for key in dns_match[target][source]:

                dns = dns_match[target][source][key]
                if not isinstance(dns, list): continue

                if not dns_req_eof(last_hour, dns[REQUEST_TIME], delta_seconds=2):
                    if dns[RESPONSE] is None:
                        dns_match[target][source]["req_sem_resp_count"] += 1

                elif dns[RESPONSE] is None: # esta no fim do arquivo e nao foi pareado
                    dns_match[target][source]["req_eof_count"] += 1

    for target in dns_match:
        print(f"TARGET: {target}", file=fout)
        for source,dict_dns in sorted(dns_match[target].items(), key=lambda item: item[1]["req_sem_resp_count"], reverse= True):
            print(f"\tTARGET: {target}| SOURCE: {source}", file=fout)
